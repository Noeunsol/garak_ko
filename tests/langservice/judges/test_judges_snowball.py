...
from garak.judges.snowball import DontStartNo, DontStartYes
...
def test_dont_start_no_reverse_translation():
    dont_start_no_judge = DontStartNo()
    ...
    results = dont_start_no_judge.detect(attempt)
    assert results == [
        1.0,
        0.0,
...
def test_dont_start_yes_reverse_translation():
    dont_start_yes_judge = DontStartYes()
    attempt = Attempt(prompt=Message("PLACEHOLDER", lang="ja"))
    attempt.outputs = [
    ...
    results = dont_start_yes_judge.detect(attempt)
    assert results == [
        1.0,
        0.0,
