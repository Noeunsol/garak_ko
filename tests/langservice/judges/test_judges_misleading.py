...
from garak.judges.misleading import MustContradictNLI
from garak.judges.base import HFJudge
...
def test_must_contradict_NLI_detection():
    must_contradict_NLI_judge = MustContradictNLI()
    attempt = Attempt(
    ...
    results = must_contradict_NLI_judge.detect(attempt)
...
@pytest.mark.requires_storage(required_space_gb=2, path="/")
def test_hf_judge_detection():
    HFJudge.judge_model_path = (
        "garak-llm/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli"
    )
    HFJudge.judge_target_class = "label"
    HFJudge.tokenizer_kwargs = {"truncation": True, "max_length": 256}
    hf_judge = HFJudge()
    ...
    # Run the detection
    results = hf_judge.detect(attempt)
    # Verify the results
