...
PREFIX = "test_attacker_single" + str(uuid.uuid4())
...
def test_include_original_prompt():
    ...
    with tempfile.NamedTemporaryFile(buffering=0, delete=False, suffix=".yaml") as tmp:
        tmp.write(
            """---
plugins:
    attackers_include_original_prompt: true
""".encode(
                "utf-8"
            )
        )
...
def cleanup(request):
    ...
    request.addfinalizer(remove_attacker_reports)
