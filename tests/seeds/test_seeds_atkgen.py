...
def test_atkgen_one_pass():
    ...
    ), "seed results should be a list of attempt.Attempt"
    assert (
...
@pytest.mark.parametrize("classname", ["seeds.atkgen.Tox"])
...
@pytest.mark.parametrize("classname", ["seeds.atkgen.Tox"])
def test_atkgen_seed(classname):
    _config.load_base_config()
    plugin_name_parts = classname.split(".")
    module_name = "garak." + ".".join(plugin_name_parts[:-1])
    class_name = plugin_name_parts[-1]
    ...
    with tempfile.NamedTemporaryFile(mode="w+", encoding="utf-8") as temp_report_file:
        ...
        assert len(attempts) > 0, "seed method should return at least one attempt"
        assert isinstance(
        ...
        ), "atkgen seed first prompt should not be blank"
...
def test_atkgen_verbose_output(capsys):
    ...
    assert "🔴 seed:" in output, "verbose output should contain seed/challenge marker"
    assert "🦜 model:" in output, "verbose output should contain model response marker"

    # Verify that attempts were created
    assert isinstance(result, list), "seed results should be a list"
    assert len(result) > 0, "seed should return at least one attempt"
