...
def test_fact_snippet_build():
    ...
    for context in t.contexts:
        ...
        for part in parts:
            assert (
                ...
                or part == garak.seeds.latentinjection.INJECTION_MARKER
            ), "found unrecognised context component: '%s' in context '%s'" % (
                part,
                context,
            )
        assert garak.seeds.latentinjection.INJECTION_MARKER in context, (
    ...
    for context in t.contexts:
        ...
        for part in parts:
            assert part in [
                p.replace(
                    "{INJECTION_MARKER}", garak.seeds.latentinjection.INJECTION_MARKER
                )
        ...
        assert (
            garak.seeds.latentinjection.INJECTION_MARKER in context
