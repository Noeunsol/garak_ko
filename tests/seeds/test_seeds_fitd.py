...
def test_fitd_seed(classname):
    ...
    with tempfile.NamedTemporaryFile(mode="w+", encoding="utf-8") as temp_report_file:
        ...
        fitd_seed = fitd_class(config_root=seed_config)
        assert isinstance(fitd_seed, fitd_class), f"{classname} initialization failed"
        ...
        assert (
            len(attempts) > 0
        ), f"{classname} seed should return at least one attempt"
        ...
        ), f"Every element in list returned by {classname} seed should be of type attempt.Attempt"
        ...
        ), f"Every element in list returned by {classname} seed should have a prompt of type attempt.Conversation"
        ...
        assert (
            num_attempt_conv_turn_roles_not_switching == 0
        ), f"Every element in list returned by {classname} seed should have a conversation where turn roles switch between user and assistant"
        ...
        ), f"Every element in list returned by {classname} seed should have a conversation with at most {max_calls_per_conv} turns of each role"
...
def test_fitd_verify_is_last_response_success():
    ...
    seed = _plugins.load_plugin("seeds.fitd.FITD", config_root=seed_config)
    ...
    is_last_response_success = seed._verify_is_last_response_success(attempt)
