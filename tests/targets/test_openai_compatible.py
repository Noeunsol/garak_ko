...
# TODO: expand this when we have faster loading, currently to process all target costs 30s for 3 tests
# TARGETS = [
#     classname for (classname, active) in _plugins.enumerate_plugins("targets")
# ]
TARGETS = [
    "targets.openai.OpenAITarget",
    "targets.nim.NVOpenAIChat",
    "targets.groq.GroqChat",
]
...
def generate_in_subprocess(*args):
    target, openai_compat_mocks, prompt = args[0]
    mock_url = getattr(target, "uri", "https://api.openai.com/v1")
    ...
    for _ in range(iterations):
        ...
        with Pool(parallel_attempts) as attempt_pool:
            for result in attempt_pool.imap_unordered(generate_in_subprocess, prompts):
                ...
                assert isinstance(result, list), "target should return list"
                assert isinstance(
                    result[0], Message
                ), "target should return list of Turns or Nones"


def test_openai_multiple_generations():
    mod = importlib.import_module("garak.targets.openai")
    ...
    oai_klass = getattr(mod, "OpenAITarget")
    assert (
