...
def test_bedrock_import():
    ...
    from garak.targets import bedrock

    assert hasattr(bedrock, "BedrockTarget")
    assert hasattr(bedrock, "DEFAULT_CLASS")
    assert bedrock.DEFAULT_CLASS == "BedrockTarget"


@pytest.mark.usefixtures("set_aws_env", "mock_boto3")
def test_bedrock_initialization():
    """Test that BedrockTarget can be initialized."""
    from garak.targets.bedrock import BedrockTarget

    target = BedrockTarget(name="claude-4-5-sonnet")
    assert target.name == "global.anthropic.claude-sonnet-4-5-20250929-v1:0"
    assert target.target_family_name == "Bedrock"
...
def test_bedrock_model_alias_resolution():
    ...
    for alias, expected_full_id in test_cases:
        target = BedrockTarget(name=alias)
        assert (
            target.name == expected_full_id
...
def test_bedrock_invalid_model_id():
    ...
    with pytest.raises(ValueError) as exc_info:
        target = BedrockTarget(name="invalid-model-name")
...
def test_bedrock_successful_generation(mock_boto3):
    ...
    output = target.generate(conv, generations_this_call=1)
...
def test_bedrock_parameter_mapping(mock_boto3):
    ...
    target.max_tokens = 100
    target.top_p = 0.9
    target.stop = ["END", "STOP"]
