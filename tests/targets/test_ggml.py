...
import pytest
import tempfile
import garak.targets.ggml

STORED_ENV = os.getenv(garak.targets.ggml.ENV_VAR)
MODEL_NAME = None


@pytest.fixture(autouse=True)
def set_fake_env() -> None:
    os.environ[garak.targets.ggml.ENV_VAR] = os.path.abspath(__file__)


def test_init_bad_app():
    with pytest.raises(RuntimeError) as exc_info:
        del os.environ[garak.targets.ggml.ENV_VAR]
        garak.targets.ggml.GgmlTarget(MODEL_NAME)
...
def test_init_bad_model():
    with tempfile.NamedTemporaryFile(
        ...
        file.close()
        with pytest.raises(RuntimeError) as exc_info:
            garak.targets.ggml.GgmlTarget(file.name)
        os.remove(file.name)
...
def test_init_good_model():
    with tempfile.NamedTemporaryFile(suffix="_test_model.gguf", delete=False) as file:
        file.write(garak.targets.ggml.GGUF_MAGIC)
        file.close()
        g = garak.targets.ggml.GgmlTarget(file.name)
        os.remove(file.name)
        assert type(g) is garak.targets.ggml.GgmlTarget
...
def test_command_args_list():
    ...
    with tempfile.NamedTemporaryFile(suffix="_test_model.gguf", delete=False) as file:
        ...
        config_root = {"targets": {"ggml": {"GgmlTarget": gen_config}}}

        g = garak.targets.ggml.GgmlTarget(file.name, config_root=config_root)
