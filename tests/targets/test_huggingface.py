...
import garak.targets.huggingface


@pytest.fixture
def hf_target_config():
    gen_config = {
        "huggingface": {
            "api_key": "fake",
            "hf_args": {
    ...
    setattr(config_root, "targets", gen_config)
    return config_root
...
def test_pipeline(hf_target_config):
    generations = 10
    g = garak.targets.huggingface.Pipeline("gpt2", config_root=hf_target_config)
    assert g.name == "gpt2"
    assert isinstance(g.target, transformers.pipelines.text_generation.Pipeline)
    assert isinstance(g.target.model, transformers.PreTrainedModel)
    assert isinstance(g.target.tokenizer, transformers.PreTrainedTokenizerBase)
...
def test_pipeline_chat(mocker, hf_target_config):
    # uses a ~350M model with chat support
    g = garak.targets.huggingface.Pipeline(
        "microsoft/DialoGPT-small", config_root=hf_target_config
    )
...
def test_inference(mocker, hf_mock_response, hf_target_config):
    target_name = "gpt2"
    mock_request = mocker.patch.object(
        requests, "request", return_value=hf_mock_response
    )

    g = garak.targets.huggingface.InferenceAPI(
        target_name, config_root=hf_target_config
    )
    assert g.name == target_name
    assert target_name in g.uri

    hf_target_config.targets["huggingface"]["name"] = target_name
    g = garak.targets.huggingface.InferenceAPI(config_root=hf_target_config)
    assert g.name == target_name
    assert target_name in g.uri
    assert isinstance(g.max_tokens, int)
    g.max_tokens = 99
...
def test_endpoint(mocker, hf_mock_response, hf_target_config):
    ...
    hf_target_config.targets["huggingface"]["name"] = target_name
    g = garak.targets.huggingface.InferenceEndpoint(config_root=hf_target_config)
    assert g.name == target_name
    assert g.uri == target_name
    assert isinstance(g.max_tokens, int)
    g.max_tokens = 99
...
def test_model(hf_target_config):
    g = garak.targets.huggingface.Model("gpt2", config_root=hf_target_config)
    assert g.name == "gpt2"
    assert isinstance(g, garak.targets.huggingface.Model)
...
def test_model_chat(mocker, hf_target_config):
    # uses a ~350M model with chat support
    g = garak.targets.huggingface.Model(
        "microsoft/DialoGPT-small", config_root=hf_target_config
    )
    mock_format = mocker.patch.object(
        g, "_conversation_to_list", wraps=g._conversation_to_list
...
def test_select_hf_device():
    from garak.targets.huggingface import HFCompatible
    import torch
