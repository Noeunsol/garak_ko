...
def test_parallel_requests():
    ...
    g = _plugins.load_plugin("targets.test.Lipsum")
    prompt = Conversation(Turn("user", [Message("this is a test")]))
    result = g.generate(prompt=prompt, generations_this_call=3)
    assert isinstance(result, list), "Target generate() should return a list"
    assert len(result) == 3, "Target should return 3 results as requested"
    assert all(
...
def test_target_structure(classname):
    ...
    assert "_call_model" in dir(
        g
    ), f"target {classname} must have a method _call_model"
    ...
    # has method generate
    assert "generate" in dir(g), f"target {classname} must have a method generate"
...
TESTABLE_TARGETS = [
    ...
    if classname
    not in [
        "targets.azure.AzureOpenAITarget",  # requires additional env variables tested in own test class
        "targets.bedrock.BedrockTarget",  # requires Bedrock-specific model names tested in own test class
        "targets.watsonx.WatsonXTarget",  # requires additional env variables tested in own test class
        "targets.function.Multiple",  # requires mock local function not implemented here
        "targets.function.Single",  # requires mock local function not implemented here
        "targets.ggml.GgmlTarget",  # validates files on disk tested in own test class
        "targets.guardrails.NeMoGuardrails",  # requires nemoguardrails as thirdy party install dependency
        "targets.huggingface.ConversationalPipeline",  # model name restrictions
        "targets.huggingface.LLaVA",  # model name restrictions
        "targets.huggingface.Model",  # model name restrictions
        "targets.huggingface.Pipeline",  # model name restrictions
        "targets.langchain.LangChainLLMTarget",  # model name restrictions
    ]
]


@pytest.mark.parametrize("classname", TESTABLE_TARGETS)
def test_instantiate_targets(classname):
    ...
    assert isinstance(g, Target)


NON_CONVERSATION_TARGETS = [
    classname
    for classname in TARGETS
...
@pytest.mark.parametrize("classname", NON_CONVERSATION_TARGETS)
def test_target_signature(classname):
    _, namespace, klass = classname.split(".")
    m = importlib.import_module(f"garak.targets.{namespace}")
    g = getattr(m, klass)
...
def test_skip_seq():
    ...
    g = _plugins.load_plugin("targets.test.Repeat")
