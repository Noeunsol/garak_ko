...
from garak.targets.litellm import LiteLLMTarget


@pytest.mark.skipif(
    not all(
        [importlib.util.find_spec(m) for m in LiteLLMTarget.extra_dependency_names]
    ),
...
def test_litellm_openai():
    ...
    target = LiteLLMTarget(name=target_name)
...
def test_litellm_openrouter():
    ...
    output = target.generate(Message("How do I write a sonnet?"))
    assert len(output) == 1  # expect 1 generation by default
...
def test_litellm_model_detection():
    ...
    target = LiteLLMTarget(name=target_name, config_root=custom_config)
    conv = Conversation(
        [Turn("user", Message("This should raise a BadTargetException"))]
    )
    with pytest.raises(BadTargetException):
        target.generate(conv)
    target = LiteLLMTarget(name="openai/invalid-model", config_root=custom_config)
    with pytest.raises(BadTargetException):
        target.generate(conv)
