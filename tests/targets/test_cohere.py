...
from garak.targets.cohere import CohereTarget, COHERE_GENERATION_LIMIT
...
def test_cohere_instantiation():
    ...
    assert hasattr(gen_v2, "target")
    assert gen_v2.api_version == "v2"

    # Test v1
    gen_v1 = CohereTarget()
    gen_v1.api_version = "v1"
    # Re-initialize the target with v1
    gen_v1.__init__()
...
def test_cohere_default_parameters():
    ...
    assert gen.temperature == CohereTarget.DEFAULT_PARAMS["temperature"]
    assert gen.max_tokens == CohereTarget.DEFAULT_PARAMS["max_tokens"]
    gen.temperature = 0.5
