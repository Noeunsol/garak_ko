...
from garak.targets.ollama import OllamaTargetChat, OllamaTarget
...
@pytest.mark.skipif(
    not all(
        [
            ...
            for m in OllamaTargetChat.extra_dependency_names
        ]
    ),
...
def test_error_on_nonexistant_model_chat():
    ...
    gen = OllamaTargetChat(model_name)
...
@pytest.mark.skipif(
    not all(
        [importlib.util.find_spec(m) for m in OllamaTarget.extra_dependency_names]
    ),
    reason="missing optional dependency",
)
@pytest.mark.skipif(
...
@pytest.mark.respx(base_url="http://" + OllamaTarget.DEFAULT_PARAMS["host"])
