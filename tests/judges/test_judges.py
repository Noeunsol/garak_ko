...
JUDGES = [
    classname
    for (classname, active) in _plugins.enumerate_plugins("judges")
    if classname
    not in [  # filter judge classes used as templates
        "judges.packagehallucination.PackageHallucinationJudge",
    ]
]
DOES_NOT_RELAY_NONE = [
    "judges.always.Fail",
...
def test_judge_structure(classname):
    ...
    # has method detect
    assert "detect" in dir(d), f"judge {classname} must have a method detect"
...
@pytest.mark.parametrize("classname", JUDGES)
def test_judge_detect(classname):
    ...
    assert isinstance(di, Judge), "judges must eventually inherit from Judge"
    assert isinstance(di, Configurable), "judges must be configurable"

    a = Attempt()
    # dressing required to get 'some' judges to return results
    a.notes["trigger"] = "x"
    a.notes["triggers"] = ["x", "x", "x", "x"]
    a.notes["repeat_word"] = "x"
    a.seed_classname = "test.Blank"

    a.prompt = Message("", lang=di.lang_spec.split(",")[0])
    a.notes["format"] = "local filename"
    a.outputs = ["test value", "", None, Message()]
    a.judge_results = {"x": [0.0, None]}
    try:
        results = di.detect(a)
    except APIKeyMissingError:
        pytest.skip(f"API key unavailable for {classname}")
    ...
    if classname == "judges.always.Passthru":
        return

    results = di.detect(a)
    if not isinstance(di, garak.judges.base.FileJudge):
        assert len(list(results)) == len(
            a.outputs
        ), "judge should return as many results as in outputs"
        if not classname in DOES_NOT_RELAY_NONE:
            for output, result in zip(a.outputs, list(results)):
                if output is not None and output.text is not None:
                    assert isinstance(
                        result, float
                    ), "judge result entries must be a float or None"
                else:
                    assert (
                        result is None
                    ), "if judge result entry is not a float, it must be a None"


@pytest.mark.parametrize("classname", JUDGES)
def test_judge_metadata(classname):
    if classname.startswith("judges.base."):
        return
    ...
    ), "judge lang must be either * or a comma-separated list of BCP47 language codes"
