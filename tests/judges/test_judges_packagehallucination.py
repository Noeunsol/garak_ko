...
import garak.judges.packagehallucination


def test_pythonpypi_init():
    d = garak.judges.packagehallucination.PythonPypi()
...
def test_rubygems_init():
    d = garak.judges.packagehallucination.RubyGems()
    assert isinstance(d, garak.judges.base.Judge)
...
def test_javascriptnpm_init():
    d = garak.judges.packagehallucination.JavaScriptNpm()
    assert isinstance(d, garak.judges.base.Judge)
...
def test_rustcrates_init():
    d = garak.judges.packagehallucination.RustCrates()
    assert isinstance(d, garak.judges.base.Judge)
...
def test_cutoff_restriction(lang):
    d = getattr(garak.judges.packagehallucination, JUDGE_NAMES[lang])()
    d.packages = None
...
def test_rakuland_judge_init():
    d = garak.judges.packagehallucination.RakuLand()
    assert isinstance(d, garak.judges.base.Judge)
...
def test_perl_judge_init():
    d = garak.judges.packagehallucination.Perl()
    assert isinstance(d, garak.judges.base.Judge)
...
def test_dart_judge_init():
    d = garak.judges.packagehallucination.Dart()
    assert isinstance(d, garak.judges.base.Judge)


def test_dart_known_package():
    judge = garak.judges.packagehallucination.Dart()
    attempt = Attempt(prompt=Message(text="Importing http"))
    attempt.outputs = ["import 'package:http/http.dart';"]
    assert judge.detect(attempt) == [
        0.0
