...
def test_filejudge_nonexist():
    d = garak.judges.base.FileJudge()
    ...
    ), "FileJudge should skip filenames for non-existing files"
...
@pytest.fixture
def string_judge():
    return garak.judges.base.StringJudge(TEST_STRINGS)


def test_case_sensitive_exact_match(string_judge):
    ...
    for test_str in TEST_STRINGS:
        ...
        results = string_judge.detect(attempt, case_sensitive=True)
        assert results == [1.0], f"Failed to detect exact match for '{test_str}'"


def test_case_sensitive_no_match(string_judge):
...
def test_case_insensitive_match(string_judge):
    ...
    for original, variant in test_cases:
        ...
        results = string_judge.detect(attempt, case_sensitive=False)
        assert results == [
            1.0
        ], f"Failed to detect case insensitive match: '{original}' vs '{variant}'"


def test_multiple_outputs(string_judge):
...
def test_none_outputs(string_judge):
    ...
    assert len(results) == len(
        attempt.outputs
    ), "Judge result for each output including None"
...
def test_matchtype_with_case_sensitivity(matchtype):
    ...
    judge = garak.judges.base.StringJudge([TEST_STRINGS[0]])
    judge.matchtype = matchtype
    ...
    for text, should_match_sensitive in MATCHTYPE_AND_CASING_CASES[matchtype]:
        ...
        # Test case sensitive
        results = judge.detect(attempt, case_sensitive=True)
        assert results == [
            1.0 if should_match_sensitive else 0.0
        ], f"Case sensitive {matchtype} match failed for '{text}'"

        # Test case insensitive
        results = judge.detect(attempt, case_sensitive=False)
        assert results == [
            1.0
        ], f"Case insensitive {matchtype} match failed for '{text}'"


def test_stringjudge_word_boundary_matching():
    judge = garak.judges.base.StringJudge(TEST_STRINGS)
    judge.matchtype = "word"
...
def test_stringjudge_startswith():
    judge = garak.judges.base.StringJudge(TEST_STRINGS)
    judge.matchtype = "startswith"
