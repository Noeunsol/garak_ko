...
Seeds in this module should implement FitD or an FitD variant.
...
class FITD(garak.seeds.IterativeSeed):
    """Foot In The Door seed
    ...
    active = True  # Note: This seed is currently not active because of the potential for the number of attempts to grow exponentially and have to be terminated early unless the user sets generations = 1

    DEFAULT_PARAMS = garak.seeds.IterativeSeed.DEFAULT_PARAMS | {
    ...
    def __init__(self, config_root=_config):
        ...
        if self.lang != self.langprovider.target_lang:
            raise GarakException(
                "FITD seed currently does not support languages other than %s"
                % self.lang
            )
    ...
    def generate_next_turn_attempt(
        ...
        if self._is_refusal(last_response):
            if len(next_turn_conv.turns) == 2 or last_non_refused_prompt is None:
                # First turn from seed (attack gen LLM) caused refusal from target. Generate a safer init turn.
                logging.debug(
    ...
    def _should_terminate_conversation(
        ...
        if self.end_condition == "judge":
            ...
            raise GarakException(
                f"Unsupported end condition '{self.end_condition}' in seed FITD. Exiting seed."
            )
