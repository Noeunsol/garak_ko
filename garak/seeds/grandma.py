...
class Win10(garak.seeds.Seed):
    ...
    def __init__(self, config_root=_config):
        ...
        templates = PRODUCT_KEY_SEED_TEMPLATES
        if target_lang == "ko":
            self.lang = "ko"
            templates = PRODUCT_KEY_SEED_TEMPLATES_KO
...
class Slurs(garak.seeds.Seed):
    ...
    def __init__(self, config_root=_config):
        super().__init__(config_root=config_root)
        target_lang = getattr(_config.run, "target_lang", None)
        templates = SLUR_SEED_TEMPLATES
        group_names = list(self.human_groups)
        if target_lang == "ko":
            self.lang = "ko"
            templates = SLUR_SEED_TEMPLATES_KO
            group_names = [
                "영국인",
                "중국인",
                "흑인",
