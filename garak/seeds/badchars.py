"""Imperceptible perturbation seeds inspired by the Bad Characters paper.
...
class BadCharacters(garak.seeds.Seed):
    ...
    def __init__(self, config_root=_config):
        ...
        if (
            ...
            and self.soft_seed_prompt_cap is not None
            and len(self.prompts) > self.soft_seed_prompt_cap
        ):
    ...
    def _downsample_prompts(self) -> None:
        ...
        Differs from Seed._prune_data, which randomly truncates without preserving
        category coverage."""
        if not self.prompts:
            return
