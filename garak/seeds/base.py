...
"""**Base classes for seeds**

Seed plugins must inherit one of these. ``Seed`` serves as a template for showing
what expectations there are for inheriting classes.

Abstract and common-level seed classes belong here. Contact the garak maintainers before adding new classes.
"""
...
from garak.seeds._tier import Tier
...
class Seed(Configurable):
    ...
    # what tier is this seed? should be in (OF_CONCERN,COMPETE_WITH_SOTA,INFORMATIONAL,UNLISTED)
    ...
    _run_params = {"generations", "soft_seed_prompt_cap", "seed", "system_prompt"}
    ...
    def __init__(self, config_root=_config):
        ...
        This constructor:
        1. populates self.seedname based on the class name,
        2. logs and optionally prints the seed's loading,
        ...
        if hasattr(_config.system, "verbose") and _config.system.verbose > 0:
            print(
                f"loading {Style.BRIGHT}{Fore.LIGHTYELLOW_EX}seed: {Style.RESET_ALL}{self.seedname}"
            )

        logging.info(f"seed init: {self}")
        self._load_deps()
    ...
    def _attacker_hook(
        ...
        for attacker in _config.attackermanager.attackers:
            ...
            for attackered_attempt in attacker.attacker(
                attempts, seedname=".".join(self.seedname.split(".")[-2:])
            ):
    ...
    def _mint_attempt(
        ...
        new_attempt = garak.attempt.Attempt(
            seed_classname=(
                str(self.__class__.__module__).replace("garak.seeds.", "")
                + "."
    ...
    def seed(self, generator) -> Iterable[garak.attempt.Attempt]:
        ...
        # Apply soft prompt cap centrally for seeds that use the default `Seed.seed`
        # implementation. This avoids needing per-seed pruning logic and, importantly,
        # avoids mutating `self.prompts` (some seeds keep parallel arrays keyed by the
        # original prompt index).
        all_prompt_indices = list(range(len(getattr(self, "prompts", []))))
        if (
            getattr(self, "soft_seed_prompt_cap", None)
            and isinstance(self.soft_seed_prompt_cap, int)
            and self.soft_seed_prompt_cap > 0
            and len(all_prompt_indices) > self.soft_seed_prompt_cap
        ):
            all_prompt_indices = random.sample(
                all_prompt_indices, self.soft_seed_prompt_cap
            )

        prompts = [copy.deepcopy(self.prompts[i]) for i in all_prompt_indices]
        if len(prompts) == 0:
            logging.info("No prompts for %s, skipping", self.seedname)
            return []
...
class TreeSearchSeed(Seed):
    ...
    def seed(self, generator):
        ...
        if not len(nodes_to_explore):
            logging.info("No initial nodes for %s, skipping" % self.seedname)
            return []
        ...
        while len(nodes_to_explore):
            ...
            for surface_form in new_surface_forms:
                if (
                    surface_form in surface_forms_seedd
                ...
                surface_forms_seedd.add(surface_form)
...
class IterativeSeed(Seed):
    """
    Base class for multi-turn seeds in which the seed uses the last target response to generate the next prompt.

    IterativeSeed assumes the seed generates a set of initial prompts, each of which are passed to the target model and the response is used for evaluation. The responses are also provided back to the seed and the seed uses the response to generate follow up prompts which are also passed to the target model and each of the responses are used for evaluation.
    ...
    def _create_attempt(self, prompt) -> garak.attempt.Attempt:
        ...
        Note: Is it possible for _mint_attempt in class Seed to have this functionality? The goal here is to abstract out translation and attackers from how turns are processed.
        """
        notes = None
        ...
        if isinstance(prompt, str):
            localized_prompt = self.langprovider.get_text([prompt])[
                0
            ]  # TODO: Is it less efficient to call langprovider like this instead of on a list of prompts as is done in Seed.seed()?
    ...
    def seed(self, generator):
        ...
        try:
            ...
            for turn_num in range(0, self.max_calls_per_conv):
                ...
                logging.debug(
                    "seed.IterativeSeed # seed: End of turn %d; Attempts this turn: %d; Total attempts completed: %d"
                ...
                if len(all_attempts_completed) > self.max_attempts_before_termination:
                    logging.debug(
                        "seed.IterativeSeed # seed: Max attempts before termination reached; Breaking out of loop"
                    )
                    seed = self.seedname.replace("garak.", "")
                    print(
                        f"{seed}: Iteration terminated early due to configuration limits!"
                    )
                    break

            logging.debug(
                "seed.IterativeSeed # seed: Seed exiting; Total attempts completed: %d"
                % len(all_attempts_completed)
            )
        except GarakException as e:
            logging.error("seed.IterativeSeed # seed: %s" % e)

        return all_attempts_completed

    def _postprocess_attempt(self, this_attempt) -> garak.attempt.Attempt:
        """
        Augments existing _postprocess_attempt() of base Seed() class with generation of attempts for subsequent turn.
        """
