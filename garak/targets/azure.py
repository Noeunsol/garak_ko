...
from garak.targets.openai import (
    OpenAICompatible,
    chat_models,
    completion_models,
    context_lengths,
...
class AzureOpenAITarget(OpenAICompatible):
    ...
    To get started with this target:
    ...
    def _load_unsafe(self):
        ...
        if self.name == "":
            raise ValueError(
                f"Deployment name is required for {self.target_family_name}, use --target_name"
            )
        ...
        if self.target_name in completion_models:
            ...
            raise ValueError(
                f"No {self.target_family_name} API defined for '{self.target_name}' in targets/openai.py - please add one!"
            )
...
DEFAULT_CLASS = "AzureOpenAITarget"
