...
from garak.exception import TargetNameMissingError, BadTargetException
from garak.targets.base import Target


class NvcfChat(Target):
    ...
    target_family_name = "NVCF"

    def __init__(self, name=None, config_root=_config):
        self.name = name
        self._load_config(config_root)
        self.fullname = (
            f"{self.target_family_name} {self.__class__.__name__} {self.name}"
        )
    ...
    def _call_model(
        ...
        if config_class in _config.plugins.targets:
            if "payload" in _config.plugins.targets[config_class]:
                for k, v in _config.plugins.targets[config_class]["payload"].items():
                    payload[k] = v
