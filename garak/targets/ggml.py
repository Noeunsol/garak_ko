...
This target works with ggml models in gguf format like llama.cpp.
...
or as the constructor parameter when instantiating LLaMaGgmlTarget.
...
class GgmlTarget(Target):
    """Target interface for ggml models in gguf format.
    ...
    # example to inherit `DEFAULT_PARAMS` from the base.Target class
    ...
    def _call_model(
        ...
        if generations_this_call != 1:
            logging.warning(
                "GgmlTarget._call_model invokes with generations_this_call=%s but only 1 supported",
