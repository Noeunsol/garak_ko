...
These judges show expectations and operations of any judge implemented
in `garak`. `garak` judges must inherit from a class in here."""
...
class Judge(Configurable):
    ...
    # we focus on LLM output for judges
    ...
    def _set_description(self):
        if "description" not in dir(self):
            try:
                ...
                err_msg: str = (
                    f"Documentation string of class {self.judgename} is unavailable, \
    ...
    def __init__(self, config_root=_config):
        ...
        self.judgename = str(self.__class__).split("'")[1]
        self._set_description()
        if hasattr(_config.system, "verbose") and _config.system.verbose > 0:
            print(
                f"loading {Style.RESET_ALL}{Fore.LIGHTBLUE_EX}judge{Style.RESET_ALL}: {self.judgename}"
            )
...
class HFJudge(Judge, HFCompatible):
    """Judge using a Hugging Face model"""
    ...
    def __init__(self, config_root=_config):
        ...
        self.name = f"{self.__class__.__name__}: {self.judge_model_path}"
...
class StringJudge(Judge):
    """Subclass of Judge using list of substrings as detection triggers"""
...
class FileJudge(Judge):
    """Judge subclass for processing attempts whose outputs are filenames for checking"""
    ...
    def detect(self, attempt: garak.attempt.Attempt) -> Iterable[float | None]:
        if self.valid_format and (
            ...
            raise ValueError(
                f"judges.fileformats.{self.__class__.__name__} only processes outputs that are '{self.valid_format}'"
            )
