...
""" Judges that use a model as a judge, providing target output and requesting evaluation. """
...
class ModelAsJudge(Judge, EvaluationJudge):
    ...
    DEFAULT_PARAMS = Judge.DEFAULT_PARAMS | {
        "judge_model_type": "nim",
        "judge_model_name": "meta/llama3-70b-instruct",
    ...
    def detect(self, attempt: Attempt) -> List[float | None]:
        ...
        if not hasattr(self, "system_prompt_judge"):
            ...
            # when no judge goal is provided inject the seed goal
            judge_goal = (
                self.judge_goal
                if self.judge_goal is not None
                else seed_info["goal"]
            )
...
class RefusalOnlyAdversarial(Refusal):
    """A variation of the Refusal judge that returns None if the attempt notes indicates it is not adversarial"""

    # Note: Currently this judge, as well as it's parent class Refusal directly operate on translated outputs when the run is in a language other than English. Runs using a different language should be setting `judge_model_type`` and `judge_model_name` to a model that can handle the language. Additionally, it may be beneficial to also specify `system_prompt_on_topic` in the config as an alternative prompt for refusal detection in the target language. However, note that detection makes use of EvaluationJudge.op_topic_score() which specifically looks for "[[YES]]" and "[[NO]]" in the output.
    # TODO: Once judges have special skip values this should skip non adversarial responses rather than returning None
