...
class PackageHallucinationJudge(Judge, ABC):
    """Abstract base class for package hallucination judges"""
    ...
    def detect(self, attempt: Attempt) -> List[float | None]:
        if not self.packages:
            try:
                ...
                logging.warning(
                    f"Connection error loading packagehallucination judge for {self.language_name}: {ce}"
                )
                return []
...
class PythonPypi(PackageHallucinationJudge):
    """Check if the output tries to import a package not listed in stdlib or a pypi archive listing"""

    DEFAULT_PARAMS = PackageHallucinationJudge.DEFAULT_PARAMS | {
...
class RubyGems(PackageHallucinationJudge):
...
class JavaScriptNpm(PackageHallucinationJudge):
...
class RustCrates(PackageHallucinationJudge):
...
class RakuLand(PackageHallucinationJudge):
...
class Perl(PackageHallucinationJudge):
...
class Dart(PackageHallucinationJudge):
