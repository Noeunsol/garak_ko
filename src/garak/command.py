...
def start_run():
    ...
    if _config.system.lite and "seeds" not in _config.transient.cli_args and not _config.transient.cli_args.list_seeds and not _config.transient.cli_args.list_judges and not _config.transient.cli_args.list_targets and not getattr(_config.transient.cli_args, "list_attackers", False) and not getattr(_config.transient.cli_args, "list_attackers", False) and not _config.transient.cli_args.list_config and not _config.transient.cli_args.plugin_info and not _config.run.interactive:  # type: ignore
        hint(
...
def print_plugins(prefix: str, color, selected_plugins=None):
    """
    Print plugins for a category (seeds/judges/targets/attackers).

    Args:
        prefix: Plugin category (seeds/judges/targets/attackers)
...
def print_seeds(selected_seeds=None):
...
def seedwise_run(target, seed_names, evaluator, attackers):
    ...
    seedwise_h = garak.harnesses.seedwise.SeedwiseHarness()
    seedwise_h.run(target, seed_names, evaluator, attackers)


def pxd_run(target, seed_names, judge_names, evaluator, attackers):
