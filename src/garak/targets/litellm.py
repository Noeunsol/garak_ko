...
When invoking garak, specify the path to the target option file:

.. code-block:: bash

   python -m garak --target_type litellm --target_name "phi" --target_option_file ollama_base.json -p dan
"""
...
from garak.exception import BadTargetException, TargetBackoffTrigger
...
class LiteLLMTarget(Target):
    """Target wrapper using LiteLLM to allow access to different providers using the OpenAI API format."""
    ...
    @backoff.on_exception(backoff.fibo, TargetBackoffTrigger, max_value=70)
    def _call_model(
        ...
        try:
            ...
            for backoff_exception in backoff_exception_types:
                if isinstance(e, backoff_exception):
                    raise TargetBackoffTrigger from e
            raise e
