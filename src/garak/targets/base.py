...
All `garak` targets must inherit from this.
"""
...
class Target(Configurable):
    ...
    def __init__(self, name="", config_root=_config):
        ...
        print(
            f"🦜 loading {Style.BRIGHT}{Fore.LIGHTMAGENTA_EX}target{Style.RESET_ALL}: {self.target_family_name}: {self.name}"
        )
        logging.info("target init: %s", self)
        self._load_deps()
    ...
    def _call_model(
        ...
        here - see garak.targets.openai for an example usage.
    ...
    def generate(
        ...
        if generations_this_call == 1:
            ...
            if (
                ...
                multi_target_bar = tqdm.tqdm(
                    total=generations_this_call,
                    leave=False,
                    colour=f"#{garak.resources.theme.TARGET_RGB}",
                )
                multi_target_bar.set_description(self.fullname[:55])
    ...
    def _conversation_to_list(conversation: Conversation) -> list[dict]:
        ...
        This is needed for a number of targets.
        """
        turn_list = [
