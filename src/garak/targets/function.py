...
Call a given function to use as a target; specify this as either the
model name on the command line, or as the parameter to the constructor.

This target is designed to be used programmatically, rather than
invoked from the CLI. An example usage might be:

.. code-block:: python

   import mymodule
   import garak.targets.function

   g = garak.targets.function.Single(name="mymodule#myfunction")
...
.. code-block:: python
   ...
   garak.cli.main("--target_type function --target_name mymodule#function_name --seeds encoding.InjectBase32".split())
...
#     function:
#       Single:
#         name: my.private.module.class#custom_target
#         kwargs:
...
#  custom_target(prompt, **kwargs)
class Single(Target):
    """Pass a function to call as a target.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.target = getattr(gen_module, gen_function_name)
        import inspect

        if "name" in inspect.signature(self.target).parameters:
            raise ValueError(
                'Incompatible function signature: "name" is incompatible with this Target'
            )
    ...
    def _call_model(
        ...
        resp_list = self.target(prompt.last_message().text, **self.kwargs)
