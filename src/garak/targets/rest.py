...
class RestTarget(Target):
    ...
    See reference docs for details (https://reference.garak.ai/en/latest/garak.targets.rest.html)
    """
    ...
    def __init__(self, uri=None, config_root=_config):
        ...
        if self.response_json:
            if self.response_json_field is None:
                raise ValueError(
                    "RestTarget response_json is True but response_json_field isn't set"
                )
            ...
            if self.response_json_field == "":
                raise ValueError(
                    "RestTarget response_json is True but response_json_field is an empty string. If the root object is the target object, use a JSONPath."
                )
        ...
        if self.method not in (
            ...
            logging.info(
                "RestTarget HTTP method %s not supported, defaulting to 'post'",
                self.method,
            )
            self.method = "post"
    ...
    @backoff.on_exception(
        backoff.fibo, (RateLimitHit, TargetBackoffTrigger), max_value=70
    )
    def _call_model(
        ...
        try:
            ...
            raise BadTargetException from uee
        ...
        if str(resp.status_code)[0] == "5":
            ...
            if self.retry_5xx:
                raise TargetBackoffTrigger(error_msg)
        ...
        if self.response_json_field[0] != "$":
            ...
            if len(responses) == 1:
                ...
                logging.error(
                    "RestTarget JSONPath in response_json_field yielded nothing. Response content: %s"
                    % repr(resp.content)
                )
                return [None]
