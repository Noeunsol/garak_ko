...
variable called COHERE_API_KEY to your Cohere API key, for this target.
...
from garak.exception import TargetBackoffTrigger
...
class CohereTarget(Target):
    ...
    def __init__(self, name="command", config_root=_config):
        ...
        if self.api_version == "v1":
            self.target = self.cohere.Client(api_key=self.api_key)
        else:  # api_version == "v2"
            self.target = self.cohere.ClientV2(api_key=self.api_key)
                    ...
                    try:
                        ...
                        for backoff_exception in backoff_exception_types:
                            if isinstance(e, backoff_exception):
                                raise TargetBackoffTrigger from e  # bubble up ApiError for backoff handling
