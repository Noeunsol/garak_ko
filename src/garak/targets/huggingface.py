...
Not all models on HF Hub work well with pipelines; try a Model target
...
class Pipeline(Target, HFCompatible):
    ...
    target_family_name = "Hugging Face 🤗 pipeline"
    ...
    def _load_unsafe(self):
        ...
        self.target = pipeline("text-generation", **pipeline_kwargs)
        if self.target.tokenizer is None:
            # account for possible model without a stored tokenizer
            from transformers import AutoTokenizer

            self.target.tokenizer = AutoTokenizer.from_pretrained(
                pipeline_kwargs["model"]
            )
        self.tokenizer = self.target.tokenizer
        if not hasattr(self, "use_chat"):
            self.use_chat = (
                hasattr(self.target.tokenizer, "chat_template")
                and self.target.tokenizer.chat_template is not None
            )
        ...
        self._set_hf_context_len(self.target.model.config)
    ...
    def _call_model(
        ...
        with warnings.catch_warnings():
            ...
            try:
                with torch.no_grad():
                    ...
                    raw_output = self.target(
                        formatted_prompt,
                        pad_token_id=self.target.tokenizer.eos_token_id,
        ...
        if raw_output is not None:
            ...
            ]  # target returns 10 outputs by default in __init__
...
class InferenceAPI(Target):
    """Get text generations from Hugging Face Inference API"""

    target_family_name = "Hugging Face 🤗 Inference API"
    supports_multiple_generations = True
    import requests
...
class Model(Pipeline, HFCompatible):
    ...
    target_family_name = "Hugging Face 🤗 model"
    supports_multiple_generations = True
...
class LLaVA(Target, HFCompatible):
