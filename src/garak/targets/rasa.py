...
"""Rasa REST API target interface
...
class RasaRestTarget(RestTarget):
    """API interface for RASA models

    Uses the following options from ``_config.plugins.targets["rasa.RasaRestTarget"]``:
    ...
    * ``ratelimit_codes`` - Which endpoint HTTP response codes should be caught
    ...
    strong option on the command line via --target_options, or save the
    JSON definition into a file and pass the filename to
    --target_option_file / -G. For example, if we save the above JSON into
    ``example_rasa_service.json``, we can invoke garak as:

      garak --target_type rest -G example_rasa_service.json

    This will load up the default RasaRestTarget and use the details in the
    ...
    DEFAULT_PARAMS = RestTarget.DEFAULT_PARAMS | {
        "headers": {
    ...
    target_family_name = "RASA"


DEFAULT_CLASS = "RasaRestTarget"
