"""OpenAI API Compatible targets

Supports chat + chatcompletion models. Put your API key in
an environment variable documented in the selected target. Put the name of the
model you want in either the --target_name command line parameter, or
pass it as an argument to the Target constructor.
...
class OpenAICompatible(Target):
    """Target base class for OpenAI compatible text2text restful API. Implements shared initialization and execution methods."""

    ENV_VAR = "OpenAICompatible_API_KEY".upper()  # Placeholder override when extending

    active = True
    supports_multiple_generations = False
    target_family_name = "OpenAICompatible"  # Placeholder override when extending
    ...
    _unsafe_attributes = ["client", "target"]

    def _load_unsafe(self):
        # When extending `OpenAICompatible` this method is a likely location for target application specific
        # customization and must populate self.target with an openai api compliant object
        self.client = openai.OpenAI(base_url=self.uri, api_key=self.api_key)
        if self.name in ("", None):
            raise ValueError(
                f"{self.target_family_name} requires model name to be set, e.g. --target_name org/private-model-name"
            )
        self.target = self.client.chat.completions
    ...
    def __init__(self, name="", config_root=_config):
        ...
        self.fullname = f"{self.target_family_name} {self.name}"
        ...
        if self.target not in (
            ...
            raise ValueError(
                "Unsupported model at generation time in targets/openai.py - please add a clause!"
            )
    ...
    @backoff.on_exception(
        ...
        (
            ...
            garak.exception.TargetBackoffTrigger,
        ),
        max_value=70,
    )
    def _call_model(
        ...
        if self.client is None:
            # reload client once when consuming the target
            self._load_unsafe()

        client = self.client
        target = self.target
        is_completion = target == client.completions
        ...
        if self.supports_multiple_generations:
            ...
            create_args["n"] = 1
        for arg in inspect.signature(target.create).parameters:
            if arg == "model":
        ...
        if is_completion:
            if not isinstance(prompt, Conversation) or len(prompt.turns) > 1:
                msg = (
                    f"Expected a Conversation with one Turn for {self.target_family_name} completions model {self.name}, but got {type(prompt)}. "
                    f"Returning nothing!"
                )
                logging.error(msg)
                return list()
            ...
            if isinstance(prompt, Conversation):
                ...
                msg = (
                    f"Expected a Conversation or list of dicts for {self.target_family_name} Chat model {self.name}, but got {type(prompt)} instead. "
                    f"Returning nothing!"
                )
                logging.error(msg)
                return list()
        ...
        try:
            response = target.create(**create_args)
            ...
            logging.exception(e)
            if self.retry_json:
                raise garak.exception.TargetBackoffTrigger from e
            else:
                raise e
        ...
        if not hasattr(response, "choices"):
            ...
            msg = "no .choices member in target response"
            if self.retry_json:
                raise garak.exception.TargetBackoffTrigger(msg)
            else:
                return [None]
...
class OpenAITarget(OpenAICompatible):
    """Target wrapper for OpenAI text2text models. Expects API key in the OPENAI_API_KEY environment variable"""

    ENV_VAR = "OPENAI_API_KEY"
    active = True
    target_family_name = "OpenAI"
    ...
    def _load_unsafe(self):
        ...
        if self.name == "":
            ...
            raise ValueError(
                f"Model name is required for {self.target_family_name}, use --target_name\n"
        ...
        if self.name in completion_models:
            self.target = self.client.completions
            ...
            raise ValueError(
                f"No {self.target_family_name} API defined for '{self.name}' in targets/openai.py - please add one!"
            )

        if self.__class__.__name__ == "OpenAITarget" and self.name.startswith("o"):
            msg = "'o'-class models should use openai.OpenAIReasoningTarget. Try e.g. `-m openai.OpenAIReasoningTarget` instead of `-m openai`"
            logging.error(msg)
            raise garak.exception.BadTargetException("🛑 " + msg)
...
class OpenAIReasoningTarget(OpenAITarget):
    """Target wrapper for OpenAI reasoning models, e.g. `o1` family."""
