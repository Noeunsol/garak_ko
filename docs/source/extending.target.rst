Let's build a target!
########################

Let's build a garak target. In garak, targets provide abstraction between the main code base and a dialogue system. That system is often an LLM, but can be anything that accepts and returns text (or text plus some other modality).
...
All targets in garak must descend from ``garak.targets.base.Target``. So, new targets start out like:

.. code-block:: python

    from garak.targets.base import Target

    class ReplicateTarget(Target):
...
It's important that targets always inheirt from ``garak.targets.base.Target``. Not only does this often save a lot of work - using the logic in base's ``generate()`` is helpful - plugin listing also checks the parents of each class in each plugin when choosing to show a plugin as available. If there's a class in a module in ``garak/targets/`` that doesn't inherit from Target, then it won't be listed as available.


Setting the default target
*****************************

Before we go ahead and code up the target, we need to tell garak which target to use by default in this class. This helps save users from always having to specify a model name. We do this in a default class module variable:

.. code-block:: python

    DEFAULT_CLASS = "ReplicateTarget"

When this target module is selected using ``--target_type replicate``, with ``replicate`` being the name of the Python module we're working in, the target loader (in ``garak/targets/__init__.py``) will search for the ``DEFAULT_CLASS`` constant and use its value to determine which class to instantiate.

Configuration
*************

Like many garak plugins, targets are configurable. This is achieved by the base class inheriting from ``Configurable``. So, the first code we see in a target class looks like this:
...
Here, we set reserved name ``ENV_VAR`` to the name of the environment variable to look in for an API key. This API key can also be specified by configuration, instead of environment variable, so this step is optional. If you're coding a target that doesn't need an API key, you can skip this.

The second block, ``DEFAULT_PARAMS``, lists configurable values for this target. These are values that aren't static and that makes for users to be able to adjust. It is defined by merging with the base target's configurable parameters. The contents are a dictionary, where keys are parameter names and the values are default values. The parameter names will become variables within this target object at instantiation -- and they're also the names of configuration variables that garak will look for in its YAML configs.

In this case, we set a few default inference parameters that reflect how the model is invoked; we'll set ``temperature``, ``top_p``, and ``repetition_penalty``. Setting these variables in the class also indicates that this target class supports those values, and they might be worth adjusting

Descriptive params
******************

The next things we need to define in this new class are the core target parameters, that describe a bit about how this target behaves and what default values it should have.

In the class, we'll set the ``target_family_name`` to ``Replicate``. This can be the same for every target in a module, or can vary a bit, but it should be a descriptive name that reflects what this class is for - the target family name is printed to garak users at run time. Finally, because Replicate endpoints aren't guaranteed to support a single request for multiple generations at a time, we set ``supports_multiple_generations`` false. This is also the default value, so technically we don't have to do it, but it's OK to be explicit here.
...
The configuration machinery will handle populating ``self.api_key``. Here, the code overrides the local environment variable in case we obtained ``api_key`` from somewhere else (e.g. a YAML config). We'll also import a copy of the ``replicate`` module in this instance, for local access. This is done because a garak run can involve multiple target instances.

If a target needs more complex environment variable loading and detection, or needs a different key populated from the ``ENV_VAR``, it should implement ``_validate_env_var()``. Examples of this can be found in the codebase.
...
(from garak/targets/langchain_serve.py)
...
How one handles this can vary. It's done this way here because replicate holds a ``Client()`` object, and the import there may not support if more than one ``ReplicateTarget`` needed to exist at the same time using different API keys. This is a quirk of the replicate library's design.

So in this case, we import the ``replicate`` API module after the initial validation. Finally, to give the module some persistence, it's loaded at the level of our target module, instead of just in this method. We add this to the end of ``__init__()``:
...
The core part of getting a result out of LLMs represented using the Replicate API is to submit a text prompt, and capture a single response to that. Within garak, functionality is handled by ``Target``'s private ``_call_model()`` method - and so that's what we will overload in the ``ReplicateTarget`` class.

The call is to the ``replicate`` module's ``run()`` method, which takes first the name of the particular hosted model requested - which we're tracking in ``self.name`` - and a dictionary parameter named ``input``. Relevant params are ``prompt`` for the input text; ``max_length`` for the upper limit on output generation size; ``temperature``, ``top_k`` and ``repetition_penalty`` to shape output text; and ``seed`` for random seed. We can access the instance of the ``replicate`` API module we created in the ``ReplicateTarget`` constructor.
...
Now that the pieces for our target are in place - a subclass of ``garak.targets.base.Target``, with some customisation in the constructor, and an overridden ``_call_model()`` method, plus a ``DEFAULT_CLASS`` given at module level - we can start to test.
...
.. code-block:: bash
    ...
    >>> import garak.targets.replicate
    >>>
...
.. code-block:: bash
    ...
    >>> g = garak.targets.replicate.ReplicateTarget("meta/llama-2-70b-chat")
    Traceback (most recent call last):
    File "<stdin>", line 1, in <module>
    File "/home/lderczynski/dev/garak/garak/targets/replicate.py", line 44, in __init__
        super().__init__(name, generations=generations, config_root=config_root)
    File "/home/lderczynski/dev/garak/garak/targets/base.py", line 43, in __init__
...
Oh, that's right! No API key. This stack trace is an example of how the ``Configurable`` interface (superclass in Python) handles the ``ENV_VAR`` load for the target without the developer having to do it manually. Looks like the validation exception is working as intended. Let's set up that value (maybe quit the interpreter, add it using the helpful suggestion in the exception method, and load up Python again).
...
.. code-block:: bash
    ...
    >>> import garak.targets.replicate
    >>> g = garak.targets.replicate.ReplicateTarget("meta/llama-2-70b-chat")
    🦜 loading target: Replicate: meta/llama-2-70b-chat
    >>>
...
The next step is to try some integration tests - executing garak from the command line, accessing this target. There are some pointers in :doc:`contributing`. You might need to execute garak by specifying it as a Python module, running the command from the garak root code directory. Things to test are:

* Does the new target appear in ``python -m garak --list_targets``?
* Does the target work with a test seed, via ``python -m garak -t replicate -n meta/llama-2-70b-chat -p test.Blank``?
* Do the garak tests pass? ``python -m pytest tests/``

Add some of your own tests if there are edge-case behaviours, general validation, or other things in ``__init__()``, ``_call_model()``, and other new methods that can be checked. Plugin-specific tests should go into a new file, ``tests/targets/test_[modulename].py``.

If you want to see the full, live code for the Replicate garak target, it's here: `garak/targets/replicate.py <https://github.com/NVIDIA/garak/blob/main/garak/targets/replicate.py>`_ .
...
This tutorial covered a tool that takes text as input and produces text as output. Garak supports multimodality - the kinds of format that a target supports are covered in a modality dictionary, with two keys, in and out. The default is:

.. code-block::

    modality: dict = {"in": {"text"}, "out": {"text"}}

For an example of a multimodal model, check out LLaVa in `garak.targets.huggingface <https://github.com/NVIDIA/garak/blob/main/garak/targets/huggingface.py>`_ .
