...
Uses the following options from ``_config.plugins.targets["rest.RestTarget"]``:
...
``--target_option_file`` / ``-G``. For example, if we save the above
...
This will load up the default ``RestTarget`` and use the details in the
