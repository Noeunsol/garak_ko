...
In garak, ``Target``\ s wrap any text-to-text+ system that garak will examine. This could be a raw LLM, a chatbot endpoint, or even a non-LLM dialog system. This base class defines the basic structure of garak's targets. All targets inherit from garak.targets.base.Target.

Attributes:

* name - The name of the specific target class. This is optionally also set in the constructor.
...
* active - Whether or not the class is active. Usually true, unless a target is disabled for some particular reason.
* target_family_name - Targets each belong to a family, describing a group. This is often related to the module name - for example, ``openai.py`` contains classes for working with OpenAI models, whose target_family_name is "openai".
* context_len - The number of tokens in the model context window, or None
* modality - A dictionary with two keys, "in" and "out", each holding a set of the modalities supported by the target. "in" refers to prompt expectations, and "out" refers to output. For example, a text-to-text+image model would have modality: ``dict = {"in": {"text"}, "out": {"text", "image"}}``.
* supports_multiple_generations - Whether or not the target can natively return multiple outputs from a prompt in a single function call. When set to False, the ``generate()`` method will make repeated calls, one output at a time, until the requested number of generations (in ``generations``) is reached.
...
#. **__init___()**: Class constructor. Call this from targets after doing local init. It does things like populating name variables, notifying target startup to the user, and logging target construction.
...
#. **_verify_model_result**: Validation of model output types, useful in debugging. If this fails, the target doesn't match the expectations in the rest of garak.
