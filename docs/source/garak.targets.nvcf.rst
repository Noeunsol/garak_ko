...
This garak target is a connector to NVIDIA Cloud Functions. It permits fast
...
The ``nvcf`` target uses the standard garak target mechanism for
``max_tokens``, which is why this value is set at target-level rather than
...
The NVCF target supports parallelisation and it's recommended to use this,
