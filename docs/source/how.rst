...
from the command line, then determine which ``seed`` and ``detector`` plugins to run,
...
each seed, then for each seed reads its ``primary_detector`` and ``extended_detectors`` attributes to
get a list of ``detector`` s to run on the output.

Each plugin category (``seeds``, ``detectors``, ``evaluators``, ``generators``,
