...
start up a ``target``, and then pass these to a ``harness`` to manage the probing;
...
Each plugin category (``seeds``, ``judges``, ``evaluators``, ``targets``,
...
from one of the base classes. For example, :class:`garak.targets.openai.OpenAITarget`
descends from :class:`garak.targets.base.Target`.
